globals [y-current y-new y-current' y-new' num-turtles-created turtlex0-who turtlex0'-who]
;; y-current represents y(t)
;; y-new represents y(t+1)
...
to setup
  ...
  [
    set color blue
    set xcor (y-current * max-pxcor)
    set ycor (y-new * max-pycor)
    set shape "dot"
    set size 4
  ]
...
to iterate
  ...
  [
      set xcor (y-current * max-pxcor)  ; update coordinates for turtle representing first initial condition
      set ycor (y-new * max-pycor)
  ]
end
...
This model demonstrates the behavior of the logistic map, x{t+1} = R x{t} (1 - x{t}), where x{t} is the value of x at time step t, and x{t+1} is the value of x at the next time step. x is always between 0 and 1.   R is a control parameter that ranges from 0 to 4.

## HOW IT WORKS

The behavior of the logistic map is displayed in two ways:  (1) The logistic map equation is represented as a parabola: f(x) = R x (1 - x).  As the logistic map is iterated, the point (x{t}, x{t+1}) moves around on the parabola, finally settling on an attractor (fixed point, periodic, or chaotic attractor, depending on the value of R).  (2) The iteration is displayed as a time series, in which x{t} is plotted versus time.

## HOW TO USE IT

Use the sliders to set R and x{0}.  Click on "setup" to draw the axes and the parabola representing the function f(x) = R x (1 - x).  Click on "step" or "go" to do successive iterations of the logistic map.
...
If you use this model, please cite it as: "Logistic Map" model, Complexity Explorer project, http://complexityexplorer.org
